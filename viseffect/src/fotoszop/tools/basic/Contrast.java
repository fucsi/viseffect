 ...
 * @author student
 */
public class Contrast extends ToolInterface {
	...
	public Contrast() {
		...
		menuItem.setIcon(this.getResourceMap().getIcon("icons.contrast"));
		dialog = new MyDialog("Dopasuj kontrast",
				"Zmien poziom kontrastu dla wybranego obrazka", this);
		dialog.addIntegerSlider(0, 100, 50, null);
		//dialog.addCheckbox("To White");
		//dialog.addCheckbox("To Black");
		//dialog.checkboxes.get(0).setSelected(true);
	...
	private void contrast(int offset, BufferedImage image) {
		int[] pixels = image.getRGB(0, 0, image.getWidth(), image.getHeight(),
				null, 0, image.getWidth());
		int alpha, red, green, blue;
		double power = ((double) offset * 2.0) / 100.0;
		for (int i = 0; i < pixels.length; i++) {
			...
			red = (int) (255.0 * java.lang.Math.pow( (double) red / 255.0, power));
			if (red < 0) {
				red = 0;
			} else if (red > 255) {
				red = 255;
			...
			green = (int) (255.0 * java.lang.Math.pow((double) green / 255.0, power));
			if (green < 0) {
				green = 0;
			} else if (green > 255) {
				green = 255;
			...
			blue = (int) (255.0 * java.lang.Math.pow((double) blue / 255.0, power));
			if (blue < 0) {
				blue = 0;
			} else if (blue > 255) {
				blue = 255;
		...
		if (dialog.isVisible()
			...
			if (btnString1.equals(value)) {
				...
				for (MyInternalFrame frame : frames) {
					...
					this.contrast(dialog.sliders.get(0).getValue(), image);
					frame.repaint();
				}
