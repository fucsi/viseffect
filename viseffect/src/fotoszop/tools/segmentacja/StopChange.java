 ...
 * @author kalkan
 */
public class StopChange extends WarunekStopu {

    public int limit = 1;

    public StopChange(double prog, int limit) {
        super(prog);
        this.limit = limit;
    }
    ...
    public boolean czyKoniec(int[] rgb, Region r) {
        ...
        if (r.composites == null || r.composites.isEmpty()) {
            if (r.visited) {
                return r.var/r.amount <= prog || (r.x1 - r.x0) <= limit || (r.y1 - r.y0) <= limit;
            }
            ...
            r.var = war;
            r.visited = true;
            return war/r.amount <= prog;
        } else {
            if (r.visited) {
                return r.var/r.amount <= prog;
            }
