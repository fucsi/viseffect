 ...
 * @author kalkan
 */
public class PeakMeanSquare extends ToolInterface {

    private JMenuItem menuItem;
    private MyDialog dialog;
    private double error1,  error2;

    public PeakMeanSquare() {
        menuItem = new JMenuItem("PeakMeanSquare");
        menuItem.setIcon(this.getResourceMap().getIcon("icons.errorpeak"));
        menuItem.addActionListener(this);
        dialog = new MyDialog("Peak Mean Square Computation", "Chose the original, with and without noise", this);
    ...
    private void peakMeanSquareError(List<BufferedImage> images) {
        ...
        max =(int) Math.round(max / 3.0);
        int size = M * N;
        max *= max;
        error1 = (double) (e1[0] + e1[1] + e1[2]) / max / size;
        error2 = (double) (e2[0] + e2[1] + e2[2]) / max / size;
